...
source(sprintf("%s/formats/Nix.r", sourceDir))
source(sprintf("%s/formats/JSON.r", sourceDir))
...
loadMirrorList <- function(){
  data <- jsonlite::fromJSON(sprintf("%s/../../layers/3_autopackages/json/mirrors-tooling.json", sourceDir))
...
genHash <- function(name, version, mirror) {
  ...
  #TODO not using prefetch for development sucks though if you change the format and have to recalculate the whole repo though
